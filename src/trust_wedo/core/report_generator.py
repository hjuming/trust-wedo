...
class ReportGenerator:
    ...
    def generate_markdown(self) -> str:
        ...
        md = f"""# Trust WEDO 信任評估報告

## 1. 實體概覽
- **實體 ID**: `{summary.get('entity_id', 'N/A')}`
- **信任評分 (EC)**: `{summary.get('entity_confidence', 0.0):.2f}`
- **判定結果**: `{"✅ 通過" if summary.get('eligibility') == 'pass' else "❌ 拒絕"}`
...
- **AI 快速答案**: {self.data.get('afb', {}).get('ai_quick_answer', 'N/A')}

## 3. 引用評估
- **最終決策**: `{citation.get('decision', 'N/A')}`
- **引用數量**: {len(citation.get('citations', []))}

## 4. 風險檢測
- **來源多樣性**: {graph.get('distinct_sources', 0)}
- **孤立實體**: `{"是" if graph.get('is_isolated') else "否"}`
- **單一來源風險**: `{"🔴 高" if graph.get('single_source_risk') else "🟢 低"}`
